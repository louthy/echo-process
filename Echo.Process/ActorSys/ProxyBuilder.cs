...
namespace Echo
{
    ...
    class ProxyBuilder
    {
        ...
        public static T Build<T>(ProcessId processId)
        {
            ...
            var interf = typeof(T);
            var interfInfo = interf.GetTypeInfo();
            if(!interfInfo.IsInterface)
            {
            ...
            res.GetType().GetTypeInfo().GetField("pid").SetValue(res, processId);
        ...
        static void CreateMethod(MethodInfo method, TypeBuilder typeBuilder, FieldBuilder pidField)
        {
            ...
            if (method.ReturnType.Name != "Void")
            {
                ...
                il.EmitCall(OpCodes.Call, typeof(System.Type).GetTypeInfo().GetMethod("GetTypeFromHandle"), new[] { typeof(RuntimeTypeHandle) });
            }
