...
{
    public class PausableBlockingQueueAsync<A> : IDisposable
    {
        readonly EventWaitHandle pauseWait = new AutoResetEvent(false);
        readonly EventWaitHandle inboxComplete = new AutoResetEvent(false);
        ...
        public PausableBlockingQueueAsync(int boundedCapacity)
        {
        ...
        public IDisposable ReceiveAsync<S>(S state, Func<S, A, Task<InboxDirective>> handler) =>
                ...
                try
                {
                    foreach (var item in items.GetConsumingEnumerable(token))
                    {
                        do
                        {
                            ...
                            try
                            {
                                handler(state, item).ContinueWith(t =>
                                {
                                    directive = t.Result;
                                    inboxComplete.Set();
                                }, token);
                                inboxComplete.WaitOne();
                            }
                            catch (Exception e)
                            {
                                Process.logErr(e);
