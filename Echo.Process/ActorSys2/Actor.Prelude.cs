...
{
    /// <summary>
    /// Static functionality that runs the actor processes 
    /// </summary>
        ...
        where EqS : struct, Eq<S>
    {
       ...
       /// <returns></returns>
        ...
        static Pipe<RT, Channel<X>, X, Unit> channelPipe<X>() =>
            from c in Proxy.awaiting<Channel<X>>()
            from r in runtime<RT>()
            from _ in Proxy.enumerate(channelYield(c, r))
            select unit;
        ...
        static Pipe<RT, Channel<SysPost>, SysPost, Unit> sysChannelPipe =>
            from c in Proxy.awaiting<Channel<SysPost>>()
            from r in runtime<RT>()
            from _ in Proxy.enumerate(sysChannelYield(c, r))
            select unit;
