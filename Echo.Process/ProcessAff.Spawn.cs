...
namespace Echo
{
        ...
        where RT : struct, HasEcho<RT>
    {
        ...
        /// <returns>A ProcessId that identifies the child</returns>
        public static Aff<RT, ProcessId> spawn<S, T>(ProcessName Name,
            Func<Aff<RT, S>> Setup,
            ...
            bool Lazy = false) =>
                ...
                from par in ActorContextAff<RT>.ParentProcess.Match(SuccessEff, _ => ActorContextAff<RT>.SelfProcess).Flatten()
                from res in ActorSystemAff<RT>.actorCreate(par, Name, Inbox, Setup, Shutdown, Terminated, Strategy, Flags, MaxMailboxSize, Lazy)
                select res;
